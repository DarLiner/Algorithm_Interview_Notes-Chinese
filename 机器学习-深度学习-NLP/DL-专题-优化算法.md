...
- 梯度下降是一种**优化算法**，通过**迭代**的方式寻找模型的**最优参数**；所谓最优参数指的是使**目标函数**达到最小值时的参数。
- **梯度**指出了函数**增长最快的方向**；梯度下降法的核心就是沿着目标函数的**负梯度**方向前进，使函数值**尽可能**减小；
  > 注意：梯度仅指示了各点处的函数值减小最多的方向；无法保证梯度所指的方向就是函数为了达到最小值应该前进的方向。
...
## 带动量的 SGD 算法
> 《深度学习》 8.3.2 动量
- 引入**动量**的目的是为了**加速** SGD 的学习过程——特别是针对高曲率、小但一致的梯度，或是带噪声的梯度
