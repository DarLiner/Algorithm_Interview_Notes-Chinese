...
- [1. **问题列表**](#1-问题列表)
- [2. 如何设置网络的初始值？*](#2-如何设置网络的初始值)
- [3. 梯度爆炸的解决办法***](#3-梯度爆炸的解决办法)
- [4. 神经网络（MLP）的万能近似定理*](#4-神经网络mlp的万能近似定理)
- [5. 神经网络中，深度与宽度的关系，及其表示能力的差异**](#5-神经网络中深度与宽度的关系及其表示能力的差异)
- [6. 在深度神经网络中，引入了隐藏层（非线性单元），放弃了训练问题的凸性，其意义何在？**](#6-在深度神经网络中引入了隐藏层非线性单元放弃了训练问题的凸性其意义何在)
- [7. 稀疏表示，低维表示，独立表示*](#7-稀疏表示低维表示独立表示)
- [8. 局部不变性（平滑先验）及其在基于梯度的学习上的局限性*](#8-局部不变性平滑先验及其在基于梯度的学习上的局限性)
- [9. 为什么交叉熵损失相比均方误差损失能提高以 sigmoid 和 softmax 作为激活函数的层的性能？**](#9-为什么交叉熵损失相比均方误差损失能提高以-sigmoid-和-softmax-作为激活函数的层的性能)
- [10. 分段线性单元（如 ReLU）代替 sigmoid 的利弊***](#10-分段线性单元如-relu代替-sigmoid-的利弊)
- [11. 在做正则化过程中，为什么只对权重做正则惩罚，而不对偏置做权重惩罚*](#11-在做正则化过程中为什么只对权重做正则惩罚而不对偏置做权重惩罚)
- [12. 列举常见的一些范数及其应用场景，如 L0、L1、L2、L∞、Frobenius等范数**](#12-列举常见的一些范数及其应用场景如-l0l1l2l∞frobenius等范数)
- [13. L1 和 L2 范数的异同***](#13-l1-和-l2-范数的异同)
- [14. 为什么 L1 正则化可以产生稀疏权值，L2 正则化可以防止过拟合？**](#14-为什么-l1-正则化可以产生稀疏权值l2-正则化可以防止过拟合)
    - [14.1. 为什么 L1 正则化可以产生稀疏权值，而 L2 不会？](#141-为什么-l1-正则化可以产生稀疏权值而-l2-不会)
    - [14.2. 为什么 L1 和 L2 正则化可以防止过拟合？](#142-为什么-l1-和-l2-正则化可以防止过拟合)
- [15. 简单介绍常用的激活函数，如 sigmoid、relu、softplus、tanh、RBF 及其应用场景***](#15-简单介绍常用的激活函数如-sigmoidrelusoftplustanhrbf-及其应用场景)
    - [15.1. 整流线性单元（ReLU）](#151-整流线性单元relu)
    - [15.2. sigmoid 与 tanh（双曲正切函数）](#152-sigmoid-与-tanh双曲正切函数)
    - [15.3. 其他激活函数（隐藏单元）](#153-其他激活函数隐藏单元)
    - [15.4. sigmoid 和 softplus 的一些性质](#154-sigmoid-和-softplus-的一些性质)
- [16. Jacobian 和 Hessian 矩阵及其在深度学习中的重要性*](#16-jacobian-和-hessian-矩阵及其在深度学习中的重要性)
- [17. 信息论、KL 散度（相对熵）与交叉熵**](#17-信息论kl-散度相对熵与交叉熵)
    - [17.1. 自信息与信息熵](#171-自信息与信息熵)
    - [17.2. 相对熵（KL 散度）与交叉熵](#172-相对熵kl-散度与交叉熵)
- [18. 如何避免数值计算中的上溢和下溢问题，以 softmax 为例*](#18-如何避免数值计算中的上溢和下溢问题以-softmax-为例)
- [19. 训练误差、泛化误差；过拟合、欠拟合；模型容量，表示容量，有效容量，最优容量的概念； 奥卡姆剃刀原则*](#19-训练误差泛化误差过拟合欠拟合模型容量表示容量有效容量最优容量的概念-奥卡姆剃刀原则)
    - [19.1. 过拟合的一些解决方案***](#191-过拟合的一些解决方案)
- [20. 高斯分布的广泛应用的原因**](#20-高斯分布的广泛应用的原因)
    - [20.1. 高斯分布（Gaussian distribution）](#201-高斯分布gaussian-distribution)
    - [20.2. 为什么推荐使用高斯分布？](#202-为什么推荐使用高斯分布)
- [21. 表示学习、自编码器与深度学习**](#21-表示学习自编码器与深度学习)
- [22. L1、L2 正则化与 MAP 贝叶斯推断的关系*](#22-l1l2-正则化与-map-贝叶斯推断的关系)
- [23. 什么是欠约束，为什么大多数的正则化可以使欠约束下的欠定问题在迭代过程中收敛*](#23-什么是欠约束为什么大多数的正则化可以使欠约束下的欠定问题在迭代过程中收敛)
- [24. 为什么考虑在模型训练时对输入 (隐藏单元或权重) 添加方差较小的噪声？*](#24-为什么考虑在模型训练时对输入-隐藏单元或权重-添加方差较小的噪声)
- [25. 多任务学习、参数绑定和参数共享***](#25-多任务学习参数绑定和参数共享)
    - [25.1. 多任务学习](#251-多任务学习)
    - [25.2. 参数绑定和参数共享](#252-参数绑定和参数共享)
- [26. Dropout 与 Bagging 集成方法的关系，Dropout 带来的意义与其强大的原因***](#26-dropout-与-bagging-集成方法的关系dropout-带来的意义与其强大的原因)
    - [26.1. Bagging 集成方法](#261-bagging-集成方法)
    - [26.2. Dropout](#262-dropout)
- [27. 批梯度下降法（Batch SGD）更新过程中，批的大小会带来怎样的影响**](#27-批梯度下降法batch-sgd更新过程中批的大小会带来怎样的影响)
- [28. 如何避免深度学习中的病态，鞍点，梯度爆炸，梯度弥散？***](#28-如何避免深度学习中的病态鞍点梯度爆炸梯度弥散)
    - [28.1. 病态（ill-conditioning）](#281-病态ill-conditioning)
    - [28.2. 鞍点（saddle point）](#282-鞍点saddle-point)
    - [28.3. 长期依赖与梯度爆炸、消失](#283-长期依赖与梯度爆炸消失)
- [29. SGD 以及学习率的选择方法、带动量的 SGD***](#29-sgd-以及学习率的选择方法带动量的-sgd)
    - [29.1. （批）随机梯度下降（SGD）与学习率](#291-批随机梯度下降sgd与学习率)
    - [29.2. 带动量的 SGD](#292-带动量的-sgd)
- [30. 自适应学习率算法：AdaGrad、RMSProp、Adam 等***](#30-自适应学习率算法adagradrmspropadam-等)
    - [30.1. AdaGrad](#301-adagrad)
    - [30.2. RMSProp](#302-rmsprop)
    - [30.3. Adam](#303-adam)
- [31. 基于二阶梯度的优化方法：牛顿法、共轭梯度、BFGS 等的做法*](#31-基于二阶梯度的优化方法牛顿法共轭梯度bfgs-等的做法)
- [32. 批标准化（Batch Normalization）的意义**](#32-批标准化batch-normalization的意义)
- [33. 神经网络中的卷积，以及卷积的动机：稀疏连接、参数共享、等变表示（平移不变性）***](#33-神经网络中的卷积以及卷积的动机稀疏连接参数共享等变表示平移不变性)
    - [33.1. 稀疏连接（sparse connectivity）](#331-稀疏连接sparse-connectivity)
    - [33.2. 参数共享（parameter sharing）](#332-参数共享parameter-sharing)
    - [33.3. 平移等变|不变性（translation invariant）](#333-平移等变不变性translation-invariant)
- [34. 卷积中不同零填充的影响**](#34-卷积中不同零填充的影响)
- [35. 基本卷积的变体：反卷积、空洞卷积***](#35-基本卷积的变体反卷积空洞卷积)
    - [35.1. 转置卷积|反卷积（Transposed convolution）](#351-转置卷积反卷积transposed-convolution)
    - [35.2. 空洞卷积|扩张卷积（Dilated convolution）](#352-空洞卷积扩张卷积dilated-convolution)
- [36. 池化、池化（Pooling）的作用***](#36-池化池化pooling的作用)
- [37. 卷积与池化的意义、影响（作为一种无限强的先验）**](#37-卷积与池化的意义影响作为一种无限强的先验)
- [38. RNN（循环神经网络） 的几种基本设计模式**](#38-rnn循环神经网络-的几种基本设计模式)
- [39. RNN 更新方程（前向传播公式），包括 LSTM、GRU 等***](#39-rnn-更新方程前向传播公式包括-lstmgru-等)
- [40. BPTT（back-propagation through time，通过时间反向传播）**](#40-bpttback-propagation-through-time通过时间反向传播)
- [41. 自编码器在深度学习中的意义*](#41-自编码器在深度学习中的意义)
- [42. 自编码器一些常见的变形与应用：正则自编码器、稀疏自编码器、去噪自编码器*](#42-自编码器一些常见的变形与应用正则自编码器稀疏自编码器去噪自编码器)
- [43. 半监督的思想以及在深度学习中的应用*](#43-半监督的思想以及在深度学习中的应用)
- [44. 分布式表示的概念、应用，与符号表示（one-hot 表示）的区别***](#44-分布式表示的概念应用与符号表示one-hot-表示的区别)
    - [44.1. 什么是分布式表示？](#441-什么是分布式表示)
    - [44.2. 分布式表示为什么强大？——分布式表示与符号表示](#442-分布式表示为什么强大分布式表示与符号表示)
- [45. 如何理解维数灾难？***](#45-如何理解维数灾难)
- [46. 迁移学习相关概念：多任务学习、一次学习、零次学习、多模态学习**](#46-迁移学习相关概念多任务学习一次学习零次学习多模态学习)
- [47. 图模型|结构化概率模型相关概念*](#47-图模型结构化概率模型相关概念)
- [48. 深度生成模型、受限玻尔兹曼机（RBM）相关概念*](#48-深度生成模型受限玻尔兹曼机rbm相关概念)
- [49. 深度学习在图像、语音、NLP等领域的常见作法与基本模型**](#49-深度学习在图像语音nlp等领域的常见作法与基本模型)
    - [49.1. 计算机视觉（CV）](#491-计算机视觉cv)
    - [49.2. 语音识别](#492-语音识别)
    - [49.3. 自然语言处理](#493-自然语言处理)
...
# 6. 在深度神经网络中，引入了隐藏层（非线性单元），放弃了训练问题的凸性，其意义何在？**
...
# 9. 为什么交叉熵损失相比均方误差损失能提高以 sigmoid 和 softmax 作为激活函数的层的性能？**
...
# 10. 分段线性单元（如 ReLU）代替 sigmoid 的利弊***
> 《深度学习》 6.6 小结
...
# 11. 在做正则化过程中，为什么只对权重做正则惩罚，而不对偏置做权重惩罚*
> 《深度学习》 7.1 参数范数惩罚
...
# 12. 列举常见的一些范数及其应用场景，如 L0、L1、L2、L∞、Frobenius等范数**
> 《深度学习》 2.5 范数（介绍）
...
# 14. 为什么 L1 正则化可以产生稀疏权值，L2 正则化可以防止过拟合？**

## 14.1. 为什么 L1 正则化可以产生稀疏权值，而 L2 不会？
...
# 15. 简单介绍常用的激活函数，如 sigmoid、relu、softplus、tanh、RBF 及其应用场景***
> 《深度学习》 6.3 隐藏单元
...
# 16. Jacobian 和 Hessian 矩阵及其在深度学习中的重要性*
> 《深度学习》 4.3.1 梯度之上：Jacobian 和 Hessian 矩阵


# 17. 信息论、KL 散度（相对熵）与交叉熵**
> 《深度学习》 3.13 信息论
...
# 18. 如何避免数值计算中的上溢和下溢问题，以 softmax 为例*
> 《深度学习》 4.1 上溢与下溢
...
# 19. 训练误差、泛化误差；过拟合、欠拟合；模型容量，表示容量，有效容量，最优容量的概念； 奥卡姆剃刀原则*
> 《深度学习》 5.2 容量、过拟合和欠拟合
...
## 20.1. 高斯分布（Gaussian distribution）
...
# 23. 什么是欠约束，为什么大多数的正则化可以使欠约束下的欠定问题在迭代过程中收敛*
> 《深度学习》 7.3 正则化与欠约束问题


# 24. 为什么考虑在模型训练时对输入 (隐藏单元或权重) 添加方差较小的噪声？*
> 《深度学习》 7.5 噪声鲁棒性
...
# 26. Dropout 与 Bagging 集成方法的关系，Dropout 带来的意义与其强大的原因***
...
# 27. 批梯度下降法（Batch SGD）更新过程中，批的大小会带来怎样的影响**
...
# 28. 如何避免深度学习中的病态，鞍点，梯度爆炸，梯度弥散？***
> 《深度学习》 8.2 神经网络优化中的挑战

## 28.1. 病态（ill-conditioning）
> 《深度学习》 8.2.1 病态
...
# 29. SGD 以及学习率的选择方法、带动量的 SGD***
> 《深度学习》 8.3 基本算法

## 29.1. （批）随机梯度下降（SGD）与学习率
...
# 30. 自适应学习率算法：AdaGrad、RMSProp、Adam 等***
> 《深度学习》 8.5 自适应学习率算法
...
# 33. 神经网络中的卷积，以及卷积的动机：稀疏连接、参数共享、等变表示（平移不变性）***
> 《深度学习》 9.2 动机
...
## 33.3. 平移等变|不变性（translation invariant）
...
![](../images/TIM截图20180612213144.png)
...
## 35.1. 转置卷积|反卷积（Transposed convolution）

![](../images/conv_no_padding_no_strides_transposed.gif)
> No padding, no strides, transposed

> [如何理解深度学习中的deconvolution networks？](https://www.zhihu.com/question/43609045) - 知乎 

## 35.2. 空洞卷积|扩张卷积（Dilated convolution）

![](../images/conv_dilation.gif)
...
![](../images/TIM截图20180613101802.png)
...
# 38. RNN（循环神经网络） 的几种基本设计模式**
> 《深度学习》 10.2 循环神经网络

循环神经网络中一些重要的设计模式包括以下几种：
1. （*）每个时间步都有输出，并且隐藏单元之间有循环连接的循环网络

    ![](../images/TIM截图20180613150111.png)
...
2. 每个时间步都产生一个输出，只有当前时刻的输出到下个时刻的隐藏单元之间有循环连接的循环网络
    ...
    ![](../images/TIM截图20180613150711.png)
...
3. 隐藏单元之间存在循环连接，但读取整个序列后产生单个输出的循环网络
    ...
    ![](../images/TIM截图20180613151014.png)
...
# 39. RNN 更新方程（前向传播公式），包括 LSTM、GRU 等***
...
- Elman RNN
    ...
    [![](../images/公式_20180613164438.png)](http://www.codecogs.com/eqnedit.php?latex=\begin{aligned}&space;h^{(t)}&=\tanh\left(&space;W_hx^{(t)}&plus;U_hh^{(t-1)}&plus;b_h&space;\right)\\&space;y^{(t)}&={\rm&space;softmax}\left(&space;W_yh^{(t)}&plus;b_y&space;\right)\\&space;\end{aligned})

- Jordan RNN

    [![](../images/公式_20180613164637.png)](http://www.codecogs.com/eqnedit.php?latex={\displaystyle&space;{\begin{aligned}&space;h^{(t)}&=\tanh\left(&space;W_hx^{(t)}&plus;U_hy^{(t-1)}&plus;b_h&space;\right)\\&space;y^{(t)}&={\rm&space;softmax}\left(&space;W_yh^{(t)}&plus;b_y&space;\right)\&space;\end{aligned}}})
...
# 40. BPTT（back-propagation through time，通过时间反向传播）**
...
# 42. 自编码器一些常见的变形与应用：正则自编码器、稀疏自编码器、去噪自编码器*
...
# 43. 半监督的思想以及在深度学习中的应用*
> 《深度学习》 15.3 半监督解释因果关系


# 44. 分布式表示的概念、应用，与符号表示（one-hot 表示）的区别***
> 《深度学习》 15.4 分布式表示

## 44.1. 什么是分布式表示？
- 所谓分布式表示就是用不同的特征，通过**组合**来表示不同的概念

    ![](../images/TIM截图20180614103628.png)
    ![](../images/TIM截图20180614104032.png)

    （左）是 one-hot 表示（一种稀疏表示），（右）为分布式表示
    > [神经网络如何学习分布式表示](http://baijiahao.baidu.com/s?id=1593632865778730392) - 百家号

## 44.2. 分布式表示为什么强大？——分布式表示与符号表示
...
![](../images/TIM截图20180614111241.png)
![](../images/TIM截图20180614111712.png)
...
![](../images/TIM截图20180614203447.png)
...
# 46. 迁移学习相关概念：多任务学习、一次学习、零次学习、多模态学习**
...
- 迁移学习与多任务学习的一些结构：
    ...
    ![](../images/TIM截图20180615111903.png)
    ![](../images/TIM截图20180610203703.png)
...
- 描述接力赛例子的有向图模型
    ...
    ![](../images/TIM截图20180617140439.png)
...
- 正式地说，变量 x 的有向概率模型是通过有向无环图 G（每个结点都是模型中的随机变量）和一系列**局部条件概率分布**（local conditional probability distribution）来定义的，x 的概率分布可以表示为：
    ...
    ![](../images/TIM截图20180617141056.png)
    - 其中 大P 表示结点 xi 的所有父结点

- 上述接力赛例子的概率分布可表示为：

    ![](../images/TIM截图20180617141502.png)
...
# 49. 深度学习在图像、语音、NLP等领域的常见作法与基本模型**
> 《深度学习》 12 应用
...
- **GCN 的意义**
    ...
    - 而将标准差视为 L2 范数的缩放，可以将 GCN 理解成到球壳的一种映射。这可能是一个有用的属性，因为神经网络往往更好地响应空间方向，而不是精确的位置。
        ...
        ![](../images/TIM截图20180619113501.png)
        - (左) 原始的输入数据可能拥有任意的范数。
...
- LCN 确保对比度在每个小窗口上被归一化，而不是作为整体在图像上被归一化。
    ...
    ![](../images/TIM截图20180619114844.png)
...
- 在**嵌入空间**中，经常出现在类似上下文中的词彼此接近。这通常导致具有相似含义的词变得邻近。
    ...
    ![](../images/TIM截图20180619205103.png)
...
- 编码器和解码器的想法 (Allen 1987; Chrisman 1991; Forcada and Ñeco 1997)很早就应用到了 NMT 中。
    ...
    ![](../images/TIM截图20180619214629.png)
...
- 更高效的方法是先读取整个句子或段落（以获得正在表达的上下文和焦点），然后一次翻译一个词，每次聚焦于输入句子的不同部分来收集产生下一个输出词所需的语义细节 (Bahdanau et al., 2015)——**Attention** 机制
    ...
    ![](../images/TIM截图20180619214735.png)
