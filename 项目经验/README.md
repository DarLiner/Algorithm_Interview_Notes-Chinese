...
- 首先对问题编码，然后将问题编码拼接到材料中，最后当做序列标注问题处理，但做了如下改动：
  - 对问题进行编码得到**问题向量**，但是模型中 LSTM 替换为 CNN，具体用到了“**空洞卷积**”与“**门卷积**”（卷积+**门限单元**）
    > [膨胀卷积](../深度学习/CNN.md#空洞卷积) & [门卷积](../深度学习/CNN.md#门卷积)
  - 将**问题向量**拼接到材料的**每一个词向量**中；
    - 参考论文 “**Attention is all you need**” 中的 Transformer 模型，加入 **Position Embedding**（位置向量）——[如何构造位置向量](#如何构造位置向量)
    - 加入人工提取的**共现特征**——匹配模型中常用的共现特征
