...
- **DP 更新**
  ```
  二分查找 nums[j] 在 dp 中的 upper_bound 位置
    - upper_bound 位置指的是序列中第一个大于 nums[j] 的元素所在的位置
    - C++ 中有 upper_bound 的实现，定义在 <algorithm> 中
  如果在末尾，则插入；反之则替换

  如果给定的数组中存在**重复的元素**，则需要做两次二分，分别找出 dp 中
    第一个大于等于（lower_bound） nums[j] 的元素位置，以及
    第一个大于（upper_bound） nums[j] 的元素位置
  只有当两个位置相等时，才进行插入或替换操作
  ```
