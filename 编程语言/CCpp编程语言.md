...
- 在 C/C++ 中，左值与右值分别表示位于赋值运算符(`=`)两侧的值：左边的即左值（L-value），右边的即右值（R-value）
  ```
  int b = 3;        // b 为左值，3 为右值
  int a = b;        // a 为左值，b 为右值
  char s = 'c';     // s 为左值，"abc" 为右值
  3 = a;            // err
  'c' = s;          // err
  ```
- C++ 中的表达式不是左值就是右值
- C++11 中引入了“**移动语义**”（move semantics）和“**右值引用**”（rvalue references）的概念，使左值与右值的概念更加复杂
- `L-value` 和 `R-value` 还有另一层含义
  - L-value 意为 Location-Value，表示**可寻址**——A value that has an address.
  - R-value 意为 Read-Value，表示**可读**
...
- **能被 `&` 取地址的就是左值**
   ...
   cout << &(foo+5);      // err，不能被取地址
   ```
  - 一切常数、字符等字面量都是右值，但**字符串是左值**
...
- [The lvalue/rvalue metaphor](https://josephmansfield.uk/articles/lvalue-rvalue-metaphor.html) — Joseph Mansfield 
  ...
  - 存在不属于任何对象的值（临时量），这些就是右值
    - 字面量 5 表示一个抽象的值，但它不属于任何对象；大多数字面量都是右值，**除了字符串**，但可以把字符串字面量当做右值处理
    - `x + y` 表示两个对象的和，但它也不存储在任何对象中

**`move()` 与 `forward()`**
- `std::move()` 允许你把任何表达式以“值”的方式处理（当做右值）
  - 把“对象”当做“值”来处理效率更高——比如在复制对象时，实际上在"move"内存中的内容，而不是"copy"它们
  ```Cpp
  widget foo() {  // 这里的返回值不是引用
      widget w;
      // ...
      return w;   // w 是一个左值，但会被当做右值处理，这里是隐式完成的
  }
  ```
...
**左值引用与右值引用**
- **左值引用**即通常所说的引用，一般表示对象的身份/别名。
- **右值引用**就是必须绑定到**右值**（一个临时对象、将要销毁的对象）的引用，一般表示对象的值。
- 右值引用可实现**转移语义**（Move Sementics）和**精确传递**（Perfect Forwarding）
- 右值引用主要有两个方面的目的：
